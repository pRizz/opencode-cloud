...
must_haves:
  truths:
    ...
    - "User can run `node packages/cli-node/dist/index.js --version` and see version output"
  artifacts:
    - path: "Cargo.toml"
    ...
    - path: "pnpm-workspace.yaml"
      ...
      contains: "packages/*"
    - path: "packages/cli-node/dist/index.js"
      provides: "Compiled Node CLI entry point"
      contains: "getVersionJs"
  key_links:
    ...
    - from: "packages/cli-node/dist/index.js"
      to: "packages/core/core.*.node"
      via: "NAPI native binding"
...
Output: Compilable monorepo with `cargo run -p opencode-cloud -- --version` and `node packages/cli-node/dist/index.js --version` both working.
</objective>
...
5. **packages/cli-node/src/index.ts**:
  ...
  <done>
    Rust CLI shows version/help with clap. Node CLI package structure exists with TypeScript source.
  </done>
</task>

<task type="auto">
  <name>Task 4: Build NAPI module and verify Node CLI</name>
  <files>
    packages/core/index.js
    packages/core/index.d.ts
    packages/core/core.*.node
    packages/cli-node/dist/index.js
  </files>
  <action>
Build the NAPI native binding and Node CLI to verify end-to-end Node functionality:
...
2. **Build NAPI module** (packages/core):
   ```bash
   pnpm -C packages/core build
   ```
   This compiles the Rust core library to a native Node addon (`.node` file) and generates:
   - `packages/core/index.js` - JavaScript wrapper
   - `packages/core/index.d.ts` - TypeScript type definitions
   - `packages/core/core.darwin-arm64.node` (or platform-specific equivalent)

3. **Build Node CLI** (packages/cli-node):
   ```bash
   pnpm -C packages/cli-node build
   ```
   This compiles TypeScript to JavaScript in `dist/`.

4. **Verify Node CLI works**:
   ```bash
   node packages/cli-node/dist/index.js --version
   ```
   Should output: `0.1.0`

5. **Verify justfile build target**:
   ```bash
   just build
   ```
   Should succeed for both Rust and Node builds.

Note: The NAPI build creates platform-specific binaries. Since we use compile-on-install (no prebuilt binaries shipped), the build runs on the user's machine during npm install.
  </action>
  <verify>
    - `ls packages/core/` shows index.js, index.d.ts, and core.*.node
    - `ls packages/cli-node/dist/` shows index.js
    - `node packages/cli-node/dist/index.js --version` outputs "0.1.0"
    - `just build` succeeds
  </verify>
  <done>
    NAPI module built successfully. Node CLI runs and outputs version. Both Rust and Node CLIs work end-to-end.
  </done>
</task>
...
4. Run `pnpm install` - should install all Node dependencies
5. Run `pnpm -C packages/core build` - should build NAPI module
6. Run `pnpm -C packages/cli-node build` - should build Node CLI
7. Run `node packages/cli-node/dist/index.js --version` - should output "0.1.0"
8. Run `just build` - should succeed for both Rust and Node
9. Verify file structure matches RESEARCH.md recommended layout
</verification>
...
- NAPI module builds and produces index.js, index.d.ts, and .node binary
- `node packages/cli-node/dist/index.js --version` outputs version string
- justfile targets work: `just build`, `just lint-rust`
