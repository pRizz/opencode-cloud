...
Create a GitHub Actions workflow that builds and publishes multi-arch Docker images to Docker Hub (primary) with GHCR mirror support on release.

## Overview

Add `docker-publish.yml` workflow triggered by `release/v*` tags (same as publish.yml). The workflow uses buildx for multi-architecture support and pushes versioned images to Docker Hub (`prizz/opencode-cloud-sandbox`) while maintaining a GHCR mirror (`ghcr.io/prizz/opencode-cloud-sandbox`).

## Prerequisites

- Phase 11 complete (current state)
- Docker Hub and GHCR access configured
...
jobs:
  build-and-push:
    ...
    permissions:
      contents: read
      packages: write  # Required for GHCR mirror push
...
4. **Push test:** Create test tag, verify workflow runs and image appears in Docker Hub (and GHCR mirror).
