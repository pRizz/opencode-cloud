...
must_haves:
  truths:
    ...
    - "Pull automatically falls back from GHCR to Docker Hub on failure"
  artifacts:
...
Pull implementation:
1. First attempt: pull from IMAGE_NAME_GHCR
2. On failure: log warning, attempt IMAGE_NAME_DOCKERHUB
...
cargo check -p opencode-cloud-core && cargo build -p opencode-cloud-core
  ...
  <done>
Image pull with multi-layer progress bars and automatic GHCR-to-DockerHub fallback.
  </done>
</task>
...
6. Pull tries GHCR first, then Docker Hub
</verification>
...
- Pull falls back from GHCR to Docker Hub on failure
