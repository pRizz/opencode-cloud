...
must_haves:
  truths:
    ...
    - "Development workflow verified: pnpm build succeeds and local bin/ fallback works"
  artifacts:
...
function isMusl(): boolean {
  ...
  // Check ldd output for musl (synchronous)
  try {
...
- Clear error messages guide users on what to do
  ...
  <verify>
1. `pnpm -C packages/cli-node build` compiles successfully.
2. With local binary in bin/occ: `node packages/cli-node/dist/index.js --version` works (fallback test).
  </verify>
  <done>
index.ts resolves binary from optionalDependencies platform packages with local bin/ fallback. Development workflow verified.
  </done>
</task>
