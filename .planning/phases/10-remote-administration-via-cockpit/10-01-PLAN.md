...
must_haves:
  truths:
    ...
    - "PAM users can log into Cockpit"
  artifacts:
...
<task type="auto">
  <name>Task 2: Add Cockpit packages from backports</name>
  <files>packages/core/src/docker/Dockerfile</files>
  <action>
Add a new section after GitHub CLI for Cockpit installation. Install from backports for latest version (339+):
...
# Install from backports for latest version
USER root
RUN . /etc/os-release && \
    apt-get update && \
...
2. Dockerfile installs cockpit-ws, cockpit-system, cockpit-bridge from backports
