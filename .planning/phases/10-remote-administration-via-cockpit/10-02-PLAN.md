...
must_haves:
  truths:
    ...
    - "Cockpit port is mapped to host when enabled"
  artifacts:
  ...
  key_links:
    - from: "schema.rs cockpit_enabled"
      ...
      via: "Config passed to create_container"
...
Purpose: Extend the config schema with Cockpit settings and update container creation to add the capabilities and mounts required for systemd to function inside the container.

Output: Config schema with cockpit_port/cockpit_enabled, container creation with CAP_SYS_ADMIN and proper mounts
</objective>
...
3. Update port bindings to include Cockpit when enabled:
...
4. Update exposed_ports map to include Cockpit:

```rust
let mut exposed_ports = HashMap::new();
exposed_ports.insert("3000/tcp".to_string(), HashMap::new());
if cockpit_enabled_val {
    exposed_ports.insert(format!("{}/tcp", cockpit_port_val), HashMap::new());
}
```
...
```
  ...
  <done>Container creation includes CAP_SYS_ADMIN, tmpfs mounts, cgroup bind mount, and Cockpit port mapping</done>
</task>

<task type="auto">
  <name>Task 3: Update callers of create_container</name>
  <files>packages/core/src/docker/container.rs</files>
  <action>
Search for all callers of create_container and update them to pass the new parameters. The primary caller is likely in the same file or in a setup function.

1. Find the setup_and_start function (or similar) that calls create_container and update it to accept and pass cockpit settings. If there's a helper function, update its signature too.

Look for usages like:
- setup_and_start in packages/core/src/docker/mod.rs or similar
- Any place that calls create_container

For now, add Optional parameters to create_container and default them:

Actually, looking at the existing signature, the callers already use Option types. So callers that don't pass cockpit settings will get None, and we handle that with defaults in the function.

However, we should also update the setup_and_start function (if it exists) to accept these params. Check packages/core/src/docker/mod.rs:

```rust
// In setup_and_start or similar function:
pub async fn setup_and_start(
    ...
    // ... pass to create_container
}
```

If callers don't need cockpit customization yet, they can pass None.

Also ensure the test in container.rs still compiles:
...
This test doesn't call create_container so it should be fine.
  </action>
  <verify>Run `just build` and `just test` - all compilation and tests pass</verify>
  <done>All callers of create_container updated to handle new parameters</done>
</task>
...
7. Cockpit port is mapped when cockpit_enabled is true
8. All tests pass: `just test`
9. Build succeeds: `just build`
</verification>

<success_criteria>
- Config schema extended with cockpit_port and cockpit_enabled
- Container creation updated for systemd compatibility
- Existing callers compile without changes (using None defaults)
- All tests pass
</success_criteria>
